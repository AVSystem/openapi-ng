...
/**
 * A chain item as a caller may write it. Wider than the published
 * `NamingRule`: `parse` also accepts an already-split `{ source, flags }`.
 *
 ...
 * once a caller supplied one, and the declared shape stays in the union
 * for the absent case.
 *
...
// Recognised option keys, which the native binding would otherwise
// ignore silently. Mirrors `GenerateOptions` in `src/bindings.rs`.
...
// Recognised `EmitTarget` values. The CLI validates its own copy in
// `bin/lib/parse.js`; both mirror `EmitTarget` in `index.d.ts`.
/** @type {ReadonlySet<string>} */
