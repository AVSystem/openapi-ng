...
// Recognised `Layout` runtime values; mirrors `VALID_LAYOUTS` in
// `bin/lib/parse.js` the same way `VALID_EMIT` does.
...
// Turn the native `{ result, error }` union into return-or-throw.
...
// Catch-all for anything still thrown across the binding boundary
// (loader failures, argument marshalling). Typed errors pass through.
...
function validateGenerateOptions(options) {
  ...
  if (options.layout !== undefined && !VALID_LAYOUTS.has(options.layout)) {
    throw new GenerateError({
      code: 'E_INVALID_OPTION',
      subcode: 'shape',
      message: `layout must be 'services', 'operations' or 'both'; got '${options.layout}'.`,
      warnings: [],
    });
  }
