...
/**
 * A spec fetched over https, and what its headers said about the format.
 *
...
// Matches https and http alike: `fetchInput` turns the http case into a
// typed error. This decides only whether the input is a URL or a path.
/**
 * @param {unknown} value
 * @returns {boolean}
 */
...
/**
 * Fails when any address `urlStr`'s host resolves to is private,
 * loopback or link-local. Re-checked per redirect hop.
 *
 * @param {string} urlStr
...
/**
 * The subset of `fetch` this module calls. Narrower than the global on
 * purpose: a test stub only has to accept what is actually passed.
 *
 * @typedef {(
 *   url: string,
